...
function load(app, url, file)
{
  var name = [
    ...
    , error = function() {
        return new Error(name+' controller: '+util.format.apply(util, arguments)+' in '+file);
      }
  ;
  ...
  config.routes.forEach(function(route, idx) {
    ...
    if (_.isFunction(route.middleware)) {
      app[method](vpath, route.middleware, handler);
    }
    else {
      app[method](vpath, handler);
    }
  });
}
...
module.exports.route = function(app, opts) {
  ...
  fs.readdirSync(opts.path).forEach(function(file) {
    load(app, opts.url, path.join(opts.path, file));
  });
}
