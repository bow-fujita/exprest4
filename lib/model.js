...
function createSequelize(opts)
{
  if (opts.uri) {
    return new Sequelize(opts.uri, _.omit(opts, 'uri'));
  }
  ...
  return new Sequelize(dsn.database, dsn.username, dsn.password, _.omit(opts, _.keys(dsn)));
}
